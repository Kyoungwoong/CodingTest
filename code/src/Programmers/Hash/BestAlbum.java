...
//    }
    ...
    static class Genre implements Comparable<Genre> {
        ...
        public Genre(int playCnt, int id) {
            this.playCnt = playCnt;
            this.id = id;
        }
    ...
    private static Map<String, List<Genre>> genreMap = new TreeMap<>();
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            categories.put(category, categories.getOrDefault(category, 0) + playCnt);
            if (!genreMap.containsKey(category)) {
                genreMap.put(category, new ArrayList<>());
            }
            genreMap.get(category).add(new Genre(playCnt, i));
        }

        Map<Integer, String> reverseMap = new TreeMap<>(Collections.reverseOrder());
        for (String key : categories.keySet()) {
            reverseMap.put(categories.get(key), key);
        }

        List<Integer> ans = new ArrayList<>();
        for (int play : reverseMap.keySet()) {
//            System.out.println("key: " + reverseMap.get(play) + " -> " + play);
            String category = reverseMap.get(play);
            List<Genre> list = genreMap.get(category);
